...
%\begin{myproof} 
%  
%  Consider any isomorphism $\phi$ such that $\phi(u) = v$. It is
...
%    $\sum\matij{\cC}{L(u)}{\phi(L(u)} \leq \alpha$, $u \in \vp$, since $\phi$ is
...
Based on the above theorem, a vertex $v$ is not a representative vertex
...
\subfloat[Cost Matrix]{
  ...
  db graph \protect\subref{subfig:database}, and cost
...
{\bf Proof:} Omitted due to lack of space.
%	\begin{myproof} Let $\phi$ be any isomorphism such that $\phi(u) =
%        v$.  We prove the theorem by using induction on $k$.\\ \textbf{Base
%        case:} $\nclab{0}{u} \preceq \nclab{0}{v} \iff \matij{\cC}{L(u)}{L(v)}
...
%        we know that $\cC[B][B'] \leq \alpha$, for all $k \geq 0$.
...
To check whether $v \in R(u)$, we first map $u$ to $v$ and subtract the
...
\medskip\noindent{\textbf{Computing \khop label cost}:} The minimum cost of
...
\scalebox{0.6}[0.6]{
  ...
  \newcommand\arc[4]{\ncline{#1}{#2}{#3}\ncput{\colorbox{gray!40}{#4}}}
	...
	\label{fig:Hflow}
\end{figure}
Fig.~\ref{fig:Hflow} shows the flow network required to compute the
minimum cost of matching the \khop labels $\khopl{2}{2} = 4,6 $ and
$\khopl{2}{20} = 40, 50, 60$ as shown in Table \ref{tab:khop220}. The
labels of vertices in the \khop labels are $C,D$ and $B, B, A$
respectively.  There is an edge from $s$ to each of $C, D$ with zero
cost and maximum capacity of one.  Similarly, there is an edge from each
of $A, B$ to the sink vertex $t$ with zero cost and maximum capacity of
one and two respectively. The capacity of the edge between $B$ and $t$
is two because both the vertices $40$ and $50$ have the same label $B$.
There is an edge from $C, D$ to each of $A, B$ with cost equal to the
corresponding entry in the cost matrix $C$. The maximum flow in the
network is two and the minimum cost of sending two units of flow $0.4$
is achieved by pushing a unit flow along the paths $s, C, B, t$ and $a,
D, A, t$.  Therefore, the cost of matching the labels $\khopl{2}{2}$ and
$\khopl{2}{20}$ is $0.4$. Thus, vertex $4$ with label $C$
can be matched to either $40$ or $50$ and the vertex $6$ to $60$.

\medskip\noindent{\textbf{Dominance check}: } Consider the \ncl labels
$\nclab{k+1}{u} = (X, Y)$ and  $\nclab{k+1}{v} = (X', Y')$, the cost of
matching the \khop labels $Y$ and $Y'$ can be computed using the above
the network formulation.  Finding an injective function $f\!\!:X
\rightarrow X'$ such that $x \preceq f(x)$, is equivalent to finding a
matching of size $|N(u)|$ in the bipartite graph with edges $(x, x')$,
for all $x \in X$ and $x' \preceq X'$.  The \ncl label $\nclab{k}{u}$ is
therefore dominated by $\nclab{k}{v}$ if the cost between the \khop
labels is within $\alpha$ and the size of maximum bipartite matching is
$|N(u)|$.

\medskip\noindent{\textbf{Optimization}: } The candidate pattern may contain
...
\begin{thm} Given a graph $G$, $k$, and $u \in \vg$, then 
  computing $\khopl{k}{u}$ is NP-Hard.  \\
  %KR cannot be solved in polynomial time unless
  %$P = NP$.
{\bf Proof:} Omitted due to lack of space.
%
...
%    by reducing it to KR. Therefore, $KR$ is at least as hard as $HP$.
